...
class NgsTextareaFunctionSmartyPlugin extends AbstractFunctionSmartyPlugin
{
    ...
    protected function getFunctionTemplate(array $params): string
    {
        ...
        return '<div '.$params['container_id'].' class="form-item' .$params['class_to_form_item'] .'">
                    <div class="input-field date-box col s6' . $params['class_to_input_field'] . '">
                        ...
                        <label for="'. $params['element_id'] .'">'. $params['display_name'] . '</label>' . $syncSage . '
                            <textarea' . $originalLanguage . 'id="' . $params['element_id'] . '"
                               name="' . $params['name'] . '" 
                               placeholder = "' .$params['display_name'] . '"
                               value="' . $params['innerText'] . '"' .'>'. $params['innerText'] .'</textarea>'
                            . $params['translations'] .
                    '</div>
                    <div class="icons-box">
                        ' .$params['helpText']. '

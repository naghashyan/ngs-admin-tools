...
class MediasMapper extends AbstractCmsMapper
{
    ...
    private string $GET_ITEM_IMAGES = "SELECT * FROM %s WHERE `object_key` = :itemId AND `object_type` = :itemType AND `type` = 'image' ORDER BY `is_main` DESC";
    ...
    private string $GET_ITEM_MAIN_IMAGE = "SELECT * FROM %s WHERE `object_key` = :itemId AND `object_type` = :itemType AND `type` = 'image' AND is_main=1 LIMIT 1";
    private string $GET_ITEM_IMAGE = "SELECT * FROM %s WHERE `object_key` = :itemId AND `object_type` = :itemType AND `type` = 'image' LIMIT 1";
     ...
     * @return \ngs\dal\dto\AbstractDto[]
     */
    public function getItemImage(int $itemId, string $itemType): ?array
    {
    ...
    private string $GET_ITEMS_MAIN_IMAGE = "SELECT * FROM %s WHERE `object_key` IN %s AND `object_type` = :itemType AND `type` = 'image' AND is_main=1 GROUP BY object_key";
    private string $GET_ITEMS_IMAGE = "SELECT * FROM %s WHERE `object_key` IN %s AND `object_type` = :itemType AND `type` = 'image' GROUP BY object_key";
    ...
    private string $GET_MEDIA_BY_KEY_OBJECT_TYPE_AND_MEDIA_TYPE = "SELECT `id` FROM %s WHERE `object_key` = :objectKey AND `object_type` = :objectType AND `type` = :mediaType";
    ...
    private string $GET_EXTENSION_BY_ID = "SELECT `extension` FROM %s WHERE `id` = %d";
    ...
    private string $CHANGE_IMAGE_TITLE = "UPDATE %s SET `description` = :description WHERE `medias`.id = :id";
    ...
    private string $GET_ITEM_ATTACHED_FILES = "SELECT * FROM %s WHERE `object_key` = :itemId AND `object_type` = :itemType AND `type` = 'file'";
    ...
    private string $GET_ITEM_MEDIA_BY_NAME = "SELECT * FROM %s WHERE `object_key` = :itemId AND `object_type` = :itemType AND `original_name` = :fileName";
    ...
    private string $ITEM_IMAGES_COUNT = "SELECT COUNT(*) AS count FROM %s WHERE `object_key` = :itemId AND `object_type` = :itemType";
    ...
    private string $GET_ITEM_BY_OBJECT_TYPE_AND_OBJECT_KEY_AND_DESCRIPTION = "SELECT *  FROM %s  WHERE `object_key` = :itemId AND `object_type` = :itemType AND `description` = :description";
